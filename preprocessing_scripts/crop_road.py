...
def crop_image(x, save_path, save_name):
	...
	for i in range(0, 1280, 256):
			for j in range(0, 1280, 256):
				...
				#print(str(i) + " " + str(j))
				#print(count)
				cv2.imwrite(save_path + save_name[0:len(x)-1-5] + "_"+ str(count) +".png", image[i:i+256,j:j+256])
				#print(save_path + x[0:len(x) - 4] + "_"+ str(count) +".png")



f = open("data_road.txt", "r")

data_path = "/home/bisag/Desktop/png/road/"
save_path = "/home/bisag/Desktop/cropped/road/"
for x in f:
	#print(x[0:len(x)-1-5] + ".png    " + str(os.path.exists("/home/bisag/Desktop/png/road/" + x[0:len(x)-1-5] + ".png")))
	if os.path.exists(save_path + x[0:len(x)-1-5]  + "_1.png"):
		print("Found")
	else:
		try:
			crop_image(data_path + x[0:len(x)-1-5] + ".png", save_path, x[0:len(x)-1-5])
			print("Saved")
